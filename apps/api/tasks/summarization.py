...
async def _gpt_summary(transcript: str, language: str | None) -> str:
...
async def _summarize_call(call_record_id: str) -> dict:
    async with AsyncSessionLocal() as db:
        ...
        summary = await _gpt_summary(record.transcription, record.transcription_lang)
        record.summary = summary
        record.summary_at = datetime.utcnow()
        record.summary_model = settings.OPENAI_MODEL if settings.OPENAI_API_KEY else "local-rule"
